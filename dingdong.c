/*
 * tapdelay.c — TapDelay LV2 Plugin v7
 *
 * Parannukset v7:
 *  - Analoginen sävy (EchoBoy-tyyli):
 *      * Pehmeä saturaatio (tanh) feedback-ketjussa
 *      * Wow/flutter: ~2 Hz LFO moduloi viiveaikaa (syvyys 0.06%)
 *      * Age-suodatin: feedback tummuu decayn kasvaessa
 *  - R-kanavan ensimmäinen echo pehmennetty (~-1.5 dB, 0.84×)
 *  - Shimmer skaalaus: x² → nuppi avautuu hitaasti, täysi arvo 1.0
 *
 * Portit: sama kuin v4–v6 (0–13), TTL ei muutu.
 * Lisenssi: MIT
 */
...
/* Wow/flutter: ~2.3 Hz, 0.06% syvyys nauhapeluri-tuntua varten */
#define WOW_RATE_HZ     2.3f
#define WOW_DEPTH       0.0006f

/* R-kanavan ensimmäisen echon vaimennus: 0.84 ≈ -1.5 dB */
#define RPONG_ATTEN     0.84f

/* Saturaation kovuus: suurempi = pehmeämpi (1.0=normaali, 2.0=puolet) */
...
{
    float delayed = ap->buf[ap->pos];
    float y = -ap->g * x + delayed;
    ap->buf[ap->pos] = x + ap->g * delayed;
    if (++ap->pos >= ap->delay) ap->pos = 0;
    return y;
}

/* Pehmeä saturaatio: tanh-approksimaatio (Padé), välttää expf-kutsun */
static inline float soft_sat(float x)
{
    /* tanh(x) ≈ x*(27+x²)/(27+9x²), tarkka |x|<3 */
    float x2 = x * x;
...
typedef struct {
    ...
    /* Age-suodatin: tummentaa feedbackia decayn mukaan */
    float age_lp;
} Channel;
...
/* ================================================================== */
static void register_tap(TapDelay* self, uint64_t t)
{
    if (self->tap_count > 0) {
    ...
    float mn  = 0.10f * (float)self->sample_rate;
    float mx  = 2.00f * (float)self->sample_rate;
    if (avg >= mn && avg <= mx) self->target_delay = avg;
}

/* ================================================================== */
static inline float grain_read(Channel* ch, float ratio,
                                float ph[2], float dp)
{
...
{
    const float dp = 1.0f / (float)GRAIN_SIZE;
    float va  = grain_read(ch, ratio_a, ch->sh_phase_a, dp);
    float vb  = grain_read(ch, ratio_b, ch->sh_phase_b, dp);
    float raw = va + vb * VOICE_B_LEVEL;
    float diff = raw;
    for (int k = 0; k < 4; k++)
        diff = allpass_process(&ch->ap[k], diff);
    return raw * 0.5f + diff * 0.5f;
}

static inline float delay_read_at(Channel* ch, float delay_samps)
{
    const float bsz = (float)ch->buf_size;
    float rp = (float)ch->write_pos - delay_samps;
    if (rp <  0.0f) rp += bsz;
    if (rp >= bsz)  rp -= bsz;
    uint32_t ri  = (uint32_t)rp;
    float    frc = rp - (float)ri;
...
static inline void process_channel(
    Channel* ch,
    float in, float delayed,
    int has_dry,
    float ratio_a, float ratio_b,
    float shimmer_mix,
    float a_hp, float a_lp, float one_m_lp,
    float decay, float mix, float level,
    float dry_in,
    float age_a,          /* age-suodattimen kerroin (LP HF-rolloff)   */
    float out_atten,      /* ulostulo-vaimennus R-kanavalle             */
    float* out_sample,
    float* out_write)
{
    /* HP → LP sarjassa */
    float hp_out     = a_hp * (ch->hp_state + delayed - ch->prev_delayed);
    ch->hp_state     = hp_out;
    ch->prev_delayed = delayed;
    ch->lp_state     = one_m_lp * hp_out + a_lp * ch->lp_state;
    float filtered   = ch->lp_state;

    /* Age-suodatin: tummentaa feedbackia decayn mukaan
       Mitä suurempi decay, sitä tummempaa echo → "vanha nauha" */
    ch->age_lp = age_a * ch->age_lp + (1.0f - age_a) * filtered;
    float aged = filtered;
    /* Sekoita alkuperäinen ja tummennettu decay-suhteessa */
    aged = filtered * (1.0f - decay * 0.4f) + ch->age_lp * (decay * 0.4f);

    /* Saturaatio feedback-ketjussa: pehmeä yliohjaus */
    float sat_in  = aged * SAT_DRIVE;
    float saturated = soft_sat(sat_in) / SAT_DRIVE;

    /* Shimmer */
    ch->sh_buf[ch->sh_write] = saturated;
    if (++ch->sh_write >= SH_BUF_SIZE) ch->sh_write = 0;
    float sh_out = (shimmer_mix > 0.001f) ?
                   shimmer_read(ch, ratio_a, ratio_b) : 0.0f;

    *out_write = (has_dry ? in : 0.0f)
               + saturated * decay
               + sh_out    * shimmer_mix * decay;

    *out_sample = (dry_in * (1.0f - mix)
                + saturated * mix * level
                + sh_out    * shimmer_mix * level) * out_atten;
}

/* ================================================================== */
static void init_allpass(Channel* ch)
{
    ...
    const char* bundle_path, const LV2_Feature* const* features)
{
    ...
    TapDelay* self = (TapDelay*)calloc(1, sizeof(TapDelay));
    if (!self) return NULL;
...
{
    TapDelay* self = (TapDelay*)instance;
...
static void activate(LV2_Handle instance)
{
    ...
    for (int c = 0; c < 2; c++) {
        ...
        ch->hp_state = ch->lp_state = ch->prev_delayed = ch->age_lp = 0.0f;
...
static void run(LV2_Handle instance, uint32_t n_samples)
{
    ...
    /* Shimmer skaalaus: x² → nuppi avautuu hitaasti, tarvitaan 
       enemmän kiertoa ennen kuin shimmer kuuluu voimakkaana.
       Esim. nuppi 0.5 → tehokas arvo 0.25, nuppi 0.7 → 0.49    */
    float raw_shimmer = *self->p_shimmer_mix;
    const float shimmer_mix = raw_shimmer * raw_shimmer;

    float sh_base;
    switch (shpitch) {
        case 1:  sh_base = 1.4983f; break;
        case 2:  sh_base = 0.5000f; break;
        default: sh_base = 2.0000f; break;
    }

    /* Age-suodattimen kerroin: tummenee decayn kasvaessa.
       decay=0 → a≈0.0 (ei tummentamista)
       decay=0.95 → a≈0.85 (merkittävä tummentaminen)            */
    ...
    if (self->midi_in) {
        LV2_ATOM_SEQUENCE_FOREACH(self->midi_in, ev) {
            ...
            if      (status == 0x90u && msg[2] > 0)   register_tap(self, t);
            else if (status == 0xB0u && msg[2] > 63u) register_tap(self, t);
            else if (status == 0xC0u)                  register_tap(self, t);
        }
    }
    ...
    for (uint32_t i = 0; i < n_samples; i++) {
        ...
        /* Pehmennetty viiveaika + wow/flutter */
        self->current_delay +=
        ...
        if (stereo == 0) {
            ...
            float delayed = delay_read_at(chL, d);
            process_channel(chL, in_m, delayed, 1,
                ra_l, rb_l, shimmer_mix,
                a_hp, a_lp, one_m_lp, decay, mix, level,
                in_m, age_a, 1.0f,
                &out_l, &write_l);
            ...
            /* ---- STEREO / PING-PONG ----
               L: saa kuivan signaalin, kirjoitetaan L-bufferiin
               R: EI saa kuivaa, saa vain L:n kaiun feedbackina
               → L→R→L→R ping-pong myös mono-lähteellä

               R-kanavan ensimmäinen echo pehmennetty RPONG_ATTEN:lla
               jotta se ei kuulosta liian aggressiiviselta suhteessa L:ään */
            float in_m = (in_l + in_r) * 0.5f;

            float delayed_l = delay_read_at(chR, d);
            float delayed_r = delay_read_at(chL, d);

            process_channel(chL, in_m, delayed_l, 1,
                ra_l, rb_l, shimmer_mix,
                a_hp, a_lp, one_m_lp, decay, mix, level,
                in_m, age_a, 1.0f,
                &out_l, &write_l);

            process_channel(chR, 0.0f, delayed_r, 0,
                ra_r, rb_r, shimmer_mix,
                a_hp, a_lp, one_m_lp, decay, mix, level,
                in_r, age_a, RPONG_ATTEN,
                &out_r, &write_r);
